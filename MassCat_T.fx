...
#define TexFile "MassCat_T.png"     // 画面に貼り付けるテクスチャファイル名
...
// 時間
float ftime : TIME <bool SyncInEditMode=true;>;		// 編集モード中は、停止する
float elapsed_time : ELAPSEDTIME;
float RotSpeed = radians( 360 / Speed );	// 回転速度
...
float4 PS_ScreenTex( float2 Tex: TEXCOORD0 ) : COLOR
{
    ...
    if ( PostionSta.x <= xy.x && xy.x <= PostionEnd.x &&
        ...
        float rotRad = RotSpeed * ftime;
        float2x2 rot = float2x2( cos(rotRad), -sin(rotRad), sin(rotRad), cos(rotRad) );
        float2 uv = mul(rot, lxy - float2(0.5, 0.5)) + float2(0.5, 0.5);
        Color = tex2D( TexSampler, uv );
    }
